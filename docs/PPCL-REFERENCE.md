...
| Parser correctness | **Empirically tested at scale.** Parses 42 programs exported from a live Desigo CC with zero failures, and 7,858 of 7,863 lines of Siemens' own 42-program application library. The five remaining are genuine syntax errors in that library as shipped -- a stray parenthesis, three missing commas, a statement fragment. Three parser bugs were found and fixed this way and could not have been found any other way. A third corpus closes the gap those two leave: Siemens' **115 worked examples**, one per command, which exercise the long tail production code never writes -- the whole PDL family, the adaptive pair, DC/DCR, the alarm limits, the phone commands. **424 of 425 lines parse**, the one failure being a missing comma in the vendor's own help page. Programs alone exercise 31 of the 66 commands; the examples take it to **57**, and the nine left have no worked example and no production use in any corpus here. Separately, an independent APOGEE P2 wire-capture corpus ran this parser over **2,644 PPCL lines recovered from programs running on panels at a working site: 2,644 parsed, none failed**, including 24 lines carrying a PPCL keyword as a dotted fragment of a point name. That evidence is not reproducible from this repository — see the note below the table |
...
| Commands the simulator does not model | **31 commands are executed; 24 more were declared unmodelled and warn; seven were neither** — `ADAPTM`, `ADAPTS`, `LSQ2`, `LSQDAT`, `LSTSQR`, `GETVAL`, `SETVAL` fell through to a silent no-op until 2026-09-24. Fixed by declaring them, not by modelling them: an invented adaptive output or curve fit is a confident wrong number. The warning now also names the points the command would have written, because everything downstream of a stale output is unsound |
